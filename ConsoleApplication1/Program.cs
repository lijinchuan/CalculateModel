...
AO:MA1-MA2;
AC:AO-MA(AO,5);
B0:IF (H>HF AND UP>TEETH AND TEETH>DOWN  AND V>=REF(V,1) AND V<=3*REF(V,1)) OR ((AO<=0 AND REF(AO,1)<REF(AO,2) AND MFI<REF(MFI,1)*0.9 AND V>=REF(V,1)*1.1)) THEN TRUE ELSE FALSE END;
S0:IF (REF(LF,1)>0 AND LF=0) OR ((REF(UP,1)<UP AND AO>0 AND REF(AO,1)>REF(AO,2) AND (MFI<=REF(MFI,1)*0.9 AND V>=REF(V,1)*1.1))) THEN TRUE ELSE F END;
IF B0 AND NOT S0 THEN BUY('新策略') ELSE IF stopprofit>=8 or Profit<-5 OR (NOT B0 AND S0)  THEN SELL('新策略') END END;";
            //            string code = @"N:13;
            //N1:8;
